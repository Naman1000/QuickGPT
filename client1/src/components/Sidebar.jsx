...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    const {chats, setSelectedChat, theme, setTheme,  user , 
        navigate,createNewChat, axios, setChats, fetchUsersChats, 
        setToken, token} = useAppContext()
    ...
    const deleteChat = async(e, chatId)=>{
        try {
            e.stopPropagation()
            const confirm = window.confirm("Are you sure you wanto to delete the chat")
            if(!confirm) return
    ...
    return (
        ...
        <div className={`flex flex-col h-screen min-w-72 dark:bg-gradient-to-b from-
        [#242124] to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl 
        ...
        'max-md:-translate-x-full' } ` }>
            ...
            {/* Reacent Chats */}
            {chats.length > 0 && <p className='mt-4 text-sm '>Recet Chatss</p>}
                <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3' >
                    ...
                    includes(search.toLowerCase())).map((chat)=> (
                        ...
                        border-gray-300 dark:border-[#80609F] rounded-md cursor-pointer 
                ...
                gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer 
                hover:scale-103 transition-all '>
                ...
                <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md  '>
                    ...
                    <label className='relative inline-flex cursor-pointer'>
                        ...
                        <div className='w-9 h-5 bg-gray-400 rounded-full peer-checked:bg-purple-600 transiton-all ' >
                            </div>
                            <span className='absolute left-1 top-1 w-3 bg-white rounded-full 
                ...
                <div className='flex items-center gap-3 p-3 mt-4 border border-gray-300
                 dark:border-white/15 rounded-md cursor-pointer group '>
                    <img src={assets.user_icon} className='w-7.5 rounded-full  ' alt='' />
